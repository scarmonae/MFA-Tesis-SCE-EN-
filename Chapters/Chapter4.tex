...
% \noindent The MTF measurement is carried out following the steps outlined below:
...
%     \item The chart is carefully rotated at an angle between $2^\circ$ and $10^\circ$ relative to the sensor’s pixel rows or columns to ensure accuracy in the MTF determination. The chart shown in Figure~\ref{fig:iso12233_test_chart} includes slanted edge patterns that meet this angular requirement, so it is not required to be rotated.
...
\noindent MTF processing is performed using the \href{https://imagej.net/ij/plugins/se-mtf/index.html}{SE\_MTF} plugin from \href{https://imagej.net/ij/index.html}{ImageJ}. In this procedure, the image scale is configured in physical units (e.g., lp/mm) based on the actual sensor size and number of photodetectors. Once parameters are configured, the plugin extracts the Edge Spread Function (ESF) from the image edge and, through numerical differentiation, generates the Line Spread Function (LSF). Then, a Fast Fourier Transform (FFT) is applied to the LSF to obtain the Modulation Transfer Function (MTF). Figure~\ref{fig:plugin_config1} shows the interface for configuring parameters. Based on the Region of Interest (ROI) and pixel size, it is possible to compute the required parameters for plugin configuration.\\
...
% \noindent Spectral characterization is an essential step for calibrating the colour response and determining the effective spectral sensitivity of a multispectral camera. In this work, the characterization was performed end-to-end: lens, filters, and sensor were evaluated together to obtain the combined spectral response of the complete imaging chain. Although individual component tests are possible, they were not required here because the mission demands depend on the net response of the integrated system. The resulting system-level curves provide the accuracy needed for applications in remote sensing, precision agriculture, and environmental monitoring, where reliable discrimination between wavelength bands is critical to data quality.

% \noindent
% Spectral calibration quantifies the optical system’s impulse response as a function of wavelength, yielding the end-to-end sensitivity curve $S(\lambda)$.  By exciting the complete imaging chain—lens, filter stack, and detector—with narrowband reference signals and recording the resulting digital output, one obtains a precise transfer function that links scene radiance at each $\lambda$ to sensor counts.  Knowledge of $S(\lambda)$ is indispensable for radiometric correction, band-to-band registration, and the retrieval of biophysical variables in remote sensing, precision agriculture, and environmental monitoring.  Calibrating the integrated system rather than its isolated components ensures that cumulative effects such as lens transmission, filter leakage, stray light, and detector quantum efficiency are captured, providing the spectral ground truth required to convert raw data into physically meaningful reflectance values.

% \noindent
% Spectral characterization establishes the end-to-end impulse response of the imaging chain, $S(\lambda)$, by illuminating the lens–filter–sensor assembly with narrow-band reference beams and mapping the resulting sensor counts to wavelength. The derived transfer function supports radiometric correction, band co-registration, and the retrieval of biophysical variables in remote-sensing, precision-agriculture, and environmental-monitoring applications, where accurate inter-band discrimination is essential. In this study $S(\lambda)$ is expressed in arbitrary units—conveying only \emph{relative} reflectance—because assigning an absolute scale would require an independently calibrated spectroradiometer and optical power meter traceable to radiometric standards, which lies beyond the scope of the present campaign.
...
% Once the filter combinations are defined and the optical power is adjusted at the reference value, the characterization of the target optical systems (e.g., an RGB system and/or a monochannel NIR system) is performed. The procedure is as follows:

% \begin{enumerate}
%     \item \textbf{Sensor configuration:}
%     \begin{itemize}
%         \item Gain: Set to $1$ for all measurements to avoid scaling differences between captures.
%         \item Gamma: Set to $1$ (linear) to suppress any internal nonlinearity curves.
%         \item White balance: In RGB cameras, the red and blue coefficients are set to $1$ (the green channel is set as 1 by default). This ensures that the camera does not apply color corrections.
%     \end{itemize}
...
%     \item \textbf{Exposure time adjustment:} The exposure time is set to avoid image saturation. This value is recorded, since the system's response is linearly proportional to the integration time (see Equation~\ref{eq:linear_response}).
    
%     \item \textbf{Data capture:}
%     \begin{itemize}
%         \item For each selected filter combination, $50$ consecutive images are acquired and saved in an uncompressed or minimally processed format. In this case, the format used is .npy.
...
%         \item The 50 corrected images are then averaged, generating a single image \(\mathbf{I}_{\text{filt}}\) for each filter combination (see section \ref{subsub:corriente_oscura_espectrometro}).
%     \end{itemize}
