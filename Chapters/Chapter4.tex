...
\noindent The quantile analysis confirms that none of the dark–corrected, averaged images approaches saturation for the exposure times used. Since both cameras deliver 8-bit data, the admissible range is $[0,255]$~DN; across all VIS channels and NIR frames the high-end quantile $q_{99.9}$ remains comfortably below this ceiling. In the VIS dataset, the largest observed upper quantiles occur for the expected channel–filter pairings (e.g., blue-weighted stacks in $B$, orange/red stacks in $G$ and $R$), yet even in these cases $q_{99.9}\!\ll\!255$~DN, indicating a clear headroom to the clipping threshold. The NIR results follow the same trend: $q_{99.9}$ lies $\sim\!220$–$230$~DN for the tested stacks, again leaving a safe margin within the 8-bit dynamic range.\\

% \noindent The middle quantiles ($q_{50}$) and the separation between $q_{90}$ and $q_{10}$ describe the spread of the pixel-value distributions and are consistent with the spectral selectivity of the stimuli. Narrowband or quasi-monochromatic conditions (e.g., interferential filters such as 532\,nm and 633\,nm, or tightly matched VIS stacks) concentrate the distributions: $q_{10}$ and $q_{90}$ are relatively close and the medians sit well below saturation, which is compatible with the ridgeline ECDFs and the narrow histograms reported previously. In contrast, broader pass-bands or composite stacks designed to increase throughput shift the upper quantiles upward and widen the interquantile span, as expected from their higher photon flux and wider spectral support, yet still without encroaching on the upper limit.\\
...
\noindent In the NIR system, the single-channel quantiles mirror the nominal band placement of the stacks centered near 820–840\,nm: $q_{50}$ values fall in the tens to low hundreds of DN, while $q_{90}$–$q_{99.9}$ approach but do not exceed $\sim\!230$~DN. This confirms that the exposure settings chosen for each combination were conservative enough to prevent highlight loss while still capturing the relative differences in throughput among stacks.\\

\noindent Taken together, the quantile heatmaps and line plots provide a compact, exposure-agnostic validation of the acquisition protocol. The joint observation of (i) $q_{99.9}$ consistently below 255~DN, (ii) physically coherent channel ranking per stack, and (iii) distribution widths that follow the intended pass-band design, demonstrates that the dataset is free of saturation artefacts within the regions of interest used for response estimation and is therefore suitable for reliable spectral-response calibration.\\
...
For each filter label and system, the mean signal was computed over a fixed central region of interest in the dark–corrected, averaged image. The resulting per–filter signals were normalized by the corresponding exposure time and by the spectral energy admitted by each filter stack. The central wavelengths used for plotting correspond to the measured centers of the selected stacks. The final curves are shown in Figure~\ref{fig:sr_relative_curves}. The VIS response exhibits the expected band structure with a blue, a green and a red lobe. The green channel peaks near the middle of the visible range and shows the highest relative sensitivity, followed by blue and then red. The NIR response presents a single lobe centered in the near–infrared region defined by the stacks around the 820–840~nm band.

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.98\linewidth}
        % \includegraphics[width=\linewidth]{Figures/C4/SR/vis_relative_sensitivity.pdf}
        \caption{VIS system. Relative spectral sensitivity for R, G and B channels.}
    \end{subfigure}
    \vspace{1ex}
    \begin{subfigure}[b]{0.98\linewidth}
        % \includegraphics[width=\linewidth]{Figures/C4/SR/nir_relative_sensitivity.pdf}
        \caption{NIR system. Relative spectral sensitivity (mono).}
    \end{subfigure}
    \caption{Relative spectral response curves obtained from the per–filter measurements after normalization. Error bands indicate the standard error across repeated acquisitions.}
    \label{fig:sr_relative_curves}
\end{figure}

\subsubsection*{Summary of observations}

The dark–current statistics are stable and low compared to the illuminated signal. The averaged, dark–corrected images present spatial uniformity adequate for deriving a scalar response over the central region. The manual exposure selection avoided saturation in all cases, as confirmed by the histogram grids. The relative spectral response curves are consistent with the expected spectral sensitivity of an RGB camera and a NIR monochannel sensor using the selected filter sets, and they provide a practical basis for subsequent radiometric normalization in the downstream analyses.


\section{Synthesis and Comparison with Requirements}
  % → Cross-summary: requirement ↔ result ↔ % of compliance.
  %   Discusses identified margins for improvement.

\section{Lessons Learned and Future Work}
  % → Reflections on design, setup, acquisition scripts
  %   and possible optimizations (e.g., narrower filters,
  %   camera firmware, vibration compensation, on-board AI).

% Optional: appendices with code listings, full data tables,
%           specification sheets for optical components, etc.